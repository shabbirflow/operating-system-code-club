...
{ 
	// To represent set of current pages. We use an unordered_set 
    // so that we quickly check if a page is present in set or not 
	unordered_set<int> s; 
	...
	{ 
		...
		// If the set is full then need to perform lru i.e. remove the least recently used page 
		...
		{ 
			...
			{ 
				...
				s.erase(val); // Remove the indexes page 
				 
				s.insert(pages[i]); // insert the current page 
				
				page_faults++; // Increment page faults 
				
			} 
			indexes[pages[i]] = i; // Update the current page index 
		} 
	} 
